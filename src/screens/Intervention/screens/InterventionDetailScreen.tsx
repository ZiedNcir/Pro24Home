    ...
    return (
        <ScreenContainer mode="light" scrollable paddingHorizontal={horizontalScale(18)} paddingVertical={verticalScale(12)} contentContainerStyle={{ paddingBottom: verticalScale(30) }}>
            ...
            {!isProfessional ? <Section>
                ...
                <InfoRow><SvgIcon name="fa-user-clock" size={16} color={colors.primary} /><Text variant="regularSmall" color="gray600">{intervention.scheduled_date || intervention.requested_date ? new Date(intervention.scheduled_date || intervention.requested_date).toLocaleString('fr-FR') : 'Date à confirmer'}</Text></InfoRow>
            </Section> : <Section><SectionLabel>Informations de la demande</SectionLabel><InfoRow><SvgIcon name="fa-map-marked-alt" size={16} color={colors.primary} /><Text variant="regularSmall" color="gray600">{formatDistanceBetweenCoordinates(user?.professional?.latitude, user?.professional?.longitude, intervention.address?.latitude, intervention.address?.longitude)}</Text></InfoRow>{intervention.service ? <InfoRow><SvgIcon name="fa-wrench" size={16} color={colors.primary} /><Text variant="regularSmall" color={colors.gray600}>{intervention.service.name}{intervention.sub_service?.name ? ` · ${intervention.sub_service.name}` : ''}</Text></InfoRow> : null}{intervention.price !== null && intervention.price !== undefined ? <InfoRow><SvgIcon name="fa-euro-sign" size={16} color={colors.primary} /><Text variant="regularSmall" color={colors.gray600}>{intervention.price.toFixed(2)} €</Text></InfoRow> : null}</Section>}
